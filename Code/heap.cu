...
__device__ void heapify_down(float *dist_heap, int *idx_heap, int heap_size, int root) {
    int largest = root;
    int left = 2 * root + 1;
    int right = 2 * root + 2;
...
__device__ void heapify_up(float *dist_heap, int *idx_heap, int i) {
    while (i > 0) {
        int parent = (i - 1) / 2;
...
__device__ void heap_insert(float *dist_heap, int *idx_heap, int k, float dist, int idx, int *heap_size) {
    if (*heap_size < k) {
        ...
        // If new dist < dist_heap[0] (largest in heap), replace and heapify down
        if (dist < dist_heap[0]) {
            dist_heap[0] = dist;
            idx_heap[0] = idx;
...
__device__ void heap_delete(float *dist_heap, int *idx_heap, int &heap_size, float &popped_dist, int &popped_idx) {
